...
class CreateGroupWalletUseCase {
  ...
  Future<Either<Failure, WalletEntity>> call(String name, String ownerId, int? accentArgb) async {
    if (name.trim().isEmpty) {
      return const Left(ValidationFailure('Tên ví nhóm không được để trống'));
    }
    try {
      final wallet = await repository.createGroupWallet(name.trim(), ownerId, accentArgb);
      return Right(wallet);
    } catch (e) {
